...
public class TrackScheduler extends AudioEventAdapter {
    ...
    private final BlockingQueue<Pair<MessageChannel, AudioTrack>> queue;
    ...
    public void enqueue(AudioTrack track) {
        if (!player.startTrack(track, true)) {// check if the player is free and play the track if so, else enqueue
            queue.offer(new ImmutablePair<>(linkedTextChannel, track));
    ...
    public void nextTrack() {
        ...
        if (track != null) {
            player.startTrack(track.getRight(), false);
            track.getLeft().sendMessage("Now playing: " + track.getRight().getInfo().title).queue();
        }
