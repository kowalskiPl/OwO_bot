...
public class WarframeDropTableParser {
    ...
    public Set<MissionRewards> processMissionDropTable(Elements missionDropTable) {
        List<List<Element>> listOfMissionsWithRewards = new LinkedList<>();
        List<Element> tmpElementList = new ArrayList<>();

        for (int i = 0; i < missionDropTable.size(); i++) {
            var singleLine = missionDropTable.get(i);
            if (singleLine.hasClass("blank-row")) {
                listOfMissionsWithRewards.add(new ArrayList<>(tmpElementList));
    ...
    public MissionRewards processSingleMission(List<Element> missionElements) throws WarframeRewardProcessingException {
    ...
    private void processRelicDropTable(Elements relicDropTable) {
